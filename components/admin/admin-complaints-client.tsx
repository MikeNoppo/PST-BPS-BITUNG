...
export default function AdminComplaintsClient({ initialItems, initialTotal, initialPage, pageSize }: Props) {
  ...
  const [original, setOriginal] = useState<ComplaintRow | null>(null)
  ...
  async function fetchPage(p: number) {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Gagal memuat halaman', variant: 'destructive' })
  ...
  }), [items, q, statusFilter, klasifikasiFilter])
  ...
  const save = async () => {
    ...
    try {
      ...
      if (statusChanged) {
        try {
          const notifyPayload = { statusOverride: payload.status, rtlOverride: payload.rtl, tanggalSelesaiOverride: payload.tanggalSelesai }
            ...
            <div className="space-y-6">
              ...
              <section className="space-y-4">
                <div>
                  ...
                  <Textarea id="rtl" rows={3} value={selected.rtl} onChange={e => setSelected(p => p ? { ...p, rtl: e.target.value } : p)} className="mt-1 bg-blue-950/40 border-blue-800/40 text-blue-50 placeholder:text-blue-300/40" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-blue-200">Status</Label>
                    <Select value={selected.status} onValueChange={v => setSelected(p => p ? { ...p, status: v } : p)}>
                  ...
                  <div>
                    ...
                    <Input id="selesai" type="date" value={selected.tanggalSelesai} onChange={e => setSelected(p => p ? { ...p, tanggalSelesai: e.target.value } : p)} className="mt-1 bg-blue-950/40 border-blue-800/40 text-blue-50 placeholder:text-blue-300/40" />
                  </div>
                </div>
